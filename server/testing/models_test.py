...
class TestAuthor:
    ...
    def test_requires_unique_name(self):
        ...
        with app.app_context():
            # Create an author with a unique name
            author_a = Author(name=Faker().name(), phone_number='1231144321')
            db.session.add(author_a)
            db.session.commit()

            # Attempt to create another author with a different name
            # This should succeed because the name is unique
            author_b = Author(name=Faker().name(), phone_number='1231144321')
